...
#import "UIImage+JHCreate.h"

@implementation UIImage (JHCreate)

- (UIImage * (^)(UIView *))JH_captureScreenForView{
    return ^(UIView *superView){
        UIGraphicsBeginImageContextWithOptions(CGSizeMake(superView.frame.size.width, superView.frame.size.height), NO, 0.0);//原图
...
- (UIImage *(^)(NSString *, CGSize))JH_imageWithQRCode{
        return ^(NSString *codeStr,CGSize size){
        if (!codeStr || codeStr.length == 0) return self;

        // str -> NSData
        NSData *strData = [codeStr dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:NO];
        ...
        return [self createNonInterpolatedUIImageFormCIImage:qrFilter.outputImage maxSize:size];
    };
}
...
-(UIImage *(^)(UIImage *, UIImage *, CGSize))JH_imagewithAddImage{
    return ^(UIImage *bgImage, UIImage *logoImage, CGSize size){
        if (logoImage == nil) {
            return bgImage;
        }
        BOOL opaque = 1.0;
